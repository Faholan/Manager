...
def get_optional(handler) -> t.Dict[str, str]:
    ...
    return {
        name: config.CLASS_NAMES.get(cls, cls.__name__) for name, cls in getattr(handler, "optional", {})
    }
...
def get_required(handler) -> t.Dict[str, str]:
    ...
    return {
        name: config.CLASS_NAMES.get(cls, cls.__name__) for name, cls in getattr(handler, "required", {})
    }
...
def authenticated(handler):
    ...
    async def check_and_run(request):
        ...
        if session.get("id", None) is None or session.get("remote", None) != request.remote:
